...
router.post('/low-performer-alert', requireRole('ADMIN', 'HR_MANAGER'), emailController.sendLowPerformerAlerts);

// POST /api/email/pay-anomaly-alert     — sends pay anomaly summary to HR_ALERT_EMAIL
router.post('/pay-anomaly-alert', requireRole('ADMIN', 'HR_MANAGER'), emailController.sendPayAnomalyAlert);

// POST /api/email/rsu-reminders         — sends RSU cliff reminders (vesting in 30 days)
router.post('/rsu-reminders', requireRole('ADMIN', 'HR_MANAGER'), emailController.sendRsuCliffReminders);

// POST /api/email/ai-compose            — Claude generates subject + HTML body from use case
router.post('/ai-compose', requireRole('ADMIN', 'HR_MANAGER'), emailController.aiCompose);

// POST /api/email/send-custom           — send a composed email and log it
router.post('/send-custom', requireRole('ADMIN', 'HR_MANAGER'), emailController.sendCustom);
